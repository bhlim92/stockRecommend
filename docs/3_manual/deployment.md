...
A template file [`.env.example`](file:///c:/Users/samsung/proj/stockRecommend/.env.example) has been created in the project root. To deploy, copy `.env.example` to `.env` and fill in the values.
...
To execute the pipeline daily without manual intervention on Windows, we configure a Windows Task Scheduler task using our automated execution script [`run_pipeline.bat`](file:///c:/Users/samsung/proj/stockRecommend/run_pipeline.bat).
...
4. **Action**:
   ...
   * **Program/script**: Browse and select the script: `c:\Users\samsung\proj\stockRecommend\run_pipeline.bat`
   * **Start in (optional)**: Set to `c:\Users\samsung\proj\stockRecommend` (extremely important for relative paths).
   * Click **Next**.
5. **Finish**:
